...
3. Add `InteractableObject` and a Collider to an item, then add `DraggableItemExample` to its Behaviours list. Assign Target Renderer, the transparent Drag Feedback Material, and the material's Color Property (`_BaseColor` for URP Lit, `_Color` for Built-in Standard).
...
The normal scene materials remain opaque and depth-writing. Only the actively dragged item temporarily uses `DragFeedback`, preventing camera-dependent transparent sorting from hiding stationary objects.
